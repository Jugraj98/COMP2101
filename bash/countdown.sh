...
#int signal accepting funciton
function signalInt {
	# changin sleep count to maximum
sleepCount=$numberOfSleeps
# informing user about rules
echo "Interrupting Count is not allowed in this script"
}

# quit signal accepting function
function signalQuit {
	# informing user
echo "You found the secret to exit the script"
# the shell should remain in good state r
stty sane
# exit script
exit 0 
}
...
# if trap signal int is found the script should not stop 
while [ $sleepCount -ge 1 ]; do
