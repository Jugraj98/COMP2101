...
# used nmcli to show all interface deices 
#used cut command to get only first coloumn 
# used -v in grep to invert selection to seleect only devices withou lo
IF=$(nmcli device | cut -d ' ' -f 1 | grep -Pv "^(DEVICE|lo)")
################
# Data Gathering
################
