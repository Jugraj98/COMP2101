...
-h : Help
-v : Verbose 
-d N : Debug [ N beign level which can be single digit]
EOF
exit 
;;
-v)
...
cat <<EOF
Verbose Mode is Set to ON
Try -h for more commands
EOF
# set variable for verbose
v="on"
;;
-d)
if [ ! -z $2 ] && [ $2 -ge 1 ] && [ $2 -le 9 ]; then
	# set variabe for deubg and level
	echo Debug Mode At Level $2
        d="on"
	l="$2"
	shift

else
	echo "Provide debug level 1-9"
fi
;;

*)
	echo "Use -h for help. Command arg invalid"
	;;
esac
