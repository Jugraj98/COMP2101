...
# Task 1
# test -f to check if file is present
# || continue to next command if first command failed ( to download pics.zip)
test -f ~/Pictures/pics.tgz || wget -q -O ~/Pictures/pics.tgz http://zonzorp.net/pics.tgz

#Task 2
# test -f to chek if the file is downloaded
# && to continue if first command passed
# tar -xvf to extract package -C to define directory where to extract
# rm to remove file if extraction is successfull
